...
public class Init {
	...
	public Init() {
		...
		if (CheckForUpdate()) {
			...
			Utils.Log("No update reported from the Master Server. Proceeding.");
		}
		...
		if (configResult) {
			...
			Utils.Log("Received ABORT. Please send a copy of these logs to the System Administrator.");
			System.exit(-1);
		}
	}
	
	public boolean CheckForUpdate() {
		Utils.Log("Querying master server for update data...");
	...
	public void DoUpdate() {
		Utils.Log("Preparing to update this backend via a remote source...");
	...
	public boolean LoadConfigFile(String path) {
		...
		Utils.Log("Attempting to load " + path + "... ");
		...
		try {
			...
			Utils.Log("ParserConfigurationException in Backend bootstrapper! Aborting.");
		} catch (SAXException e) {
			readSuccessful = false;
			e.printStackTrace();

			Utils.Log("SAXException in Backend bootstrapper! Aborting.");
		} catch (IOException e) {
			readSuccessful = false;
			e.printStackTrace();

			Utils.Log("IOException in Backend bootstrapper! Aborting.");
		}
	...
	public void ParseConfigFile() {
		Utils.Log("Load complete! Parsing entries into master object...");
	...
	public String GetElementByNode(String nodeName, String elementName) {
		...
		if (doc != null) {
			Utils.Log("FETCH_" + nodeName + "@" + elementName +"");
			...
			Utils.Log("Premature call to GetElementByNode! You must first initialize the Document with LoadConfigFile(). Ignoring.");
		}
		return elementValue;
	}
	...
	public class PropertyMaster {
		...
		public PropertyMaster() {
			Utils.Log("A PropertyMaster object has been allocated!");
		}
	}
}
